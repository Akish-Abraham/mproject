...
public interface UserRepository extends JpaRepository<Users, Integer> {
    ...
    //Dashboard Common
    @Query(value="SELECT avg(progress),project.projectid,users.userid,users.username,project.projectname FROM project JOIN users ON project.clientid = users.userid JOIN tasks ON project.projectid = tasks.projectid  where users.userid=? group by project.projectid;",nativeQuery = true)
